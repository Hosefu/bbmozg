...
public static class PasswordHelper
{
    ...
    public static string GeneratePassword(int length = 12, bool includeUppercase = true, bool includeLowercase = true, bool includeNumbers = true, bool includeSpecialChars = true)
    {
        ...
        if (includeUppercase)
        {
            ...
            requiredCharacters.Add(GetRandomChar(uppercase));
        }
        if (includeLowercase)
        {
            var lowercase = "abcdefghijklmnopqrstuvwxyz";
            allCharacters.Append(lowercase);
            requiredCharacters.Add(GetRandomChar(lowercase));
        }
        if (includeNumbers)
        {
            var numbers = "0123456789";
            allCharacters.Append(numbers);
            requiredCharacters.Add(GetRandomChar(numbers));
        }
        if (includeSpecialChars)
        {
            var special = "!@#$%^&*()_+-=[]{}|;:,.<>?";
            allCharacters.Append(special);
            requiredCharacters.Add(GetRandomChar(special));
        }
            
        if (allCharacters.Length == 0)
            throw new ArgumentException("Должен быть выбран хотя бы один тип символов");
            
        var charArray = allCharacters.ToString().ToCharArray();
        var password = new List<char>(requiredCharacters);
        
        // Добавляем оставшиеся символы случайно
        using var rng = RandomNumberGenerator.Create();
        for (int i = requiredCharacters.Count; i < length; i++)
        {
            password.Add(GetRandomChar(charArray));
        }
