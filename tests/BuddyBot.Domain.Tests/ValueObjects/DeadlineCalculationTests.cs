...
public class DeadlineCalculationTests
{
    ...
    public void Constructor_WithCustomWorkingDays_UsesProvidedDays()
    {
        ...
        var customWorkingDays = new List<BuddyBot.Domain.Enums.DayOfWeek>
        {
            BuddyBot.Domain.Enums.DayOfWeek.Monday,
            BuddyBot.Domain.Enums.DayOfWeek.Wednesday,
            BuddyBot.Domain.Enums.DayOfWeek.Friday
        };
    ...
    public void CalculateDeadline_WithCustomWorkingDays_OnlyCountsSpecifiedDays()
    {
        ...
        // Assert
        // 2 рабочих дня: следующий понедельник (22.01) и среда (24.01)
        var expectedDeadline = new DateTime(2024, 1, 24); // Среда
        Assert.Equal(expectedDeadline, deadline.DeadlineDate);
    }
}